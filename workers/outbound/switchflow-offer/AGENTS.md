# Offers

Current direction: [Ads + booking](installation-booking.md), Compass key `installation-booking`, testing. Start with Sydney residential ducted installations/replacements: Google Search acquisition plus enquiry qualification and booked quote appointments. Existing enquiries are not required. The contract owns scope and targeting. Compass owns operational status, copy, campaigns and results.

Use `.agents/skills/offer-create/SKILL.md` for offer changes. Jules has approved this direction and retiring conflicting defaults. Prices, tax and individual agreement terms must be explicit; do not inherit the previous guarantee or billing rules.

Historical Booked jobs (`booked-jobs-system`) and missed-call-only (`ai-receptionist-system`) are retired. Their records belong in historical review, not new campaign suggestions. See `archives/offer-transition-2026-09-08/README.md` and exact snapshots. Do not delete historical lead/contact/campaign evidence or relabel it as a result for the new offer.

Same service plus new vertical: assess fit before changing targeting. New service: use a separate contract and offer identity. Never invent proof, emails, commercial terms or performance claims. Draft campaign preparation does not authorise activation. Do not change existing clients' contracts by changing this default.
