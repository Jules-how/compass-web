...
0. Copy pointer comes from `factory_job.py` stdout: if Compass `copy_confirmed_at` is set on the cell, GET `/api/agent/outbound/campaigns/<id>/copy?full=1` and paste that `sequence_draft`. Otherwise paste the **fill** sequence from `cold-email/email-copy/fill-capture.md`.
...
4. After the CSV is on the draft, PATCH Compass `sequence_draft` on the cell (`/api/agent/outbound/campaigns/<id>/copy`) with the sequence as loaded, then `mark` the cohort `in_instantly`.
...
- Name: `{City} {Trade} | fill | {n} | {Mon}`. Always fill. One sequence per campaign.
...
Email 1 subject `{{subject}}`. Paste the **fill** Email 1 plus the shared bump from `cold-email/email-copy/fill-capture.md`. Fill `{job_type}` and `{extra_jobs}` there first. Locksmith, electrical, roofing, and pest omit the extra-job count (capacity CTA only; locksmith uses that file's after-hours / car keys line). Do not paste 10 to 15 on every vertical. Do not add a P.S. Do not add `{a|b}` unless Jules asks. Do not load capture on this path.
...
- [ ] Give, CTA, and bump match the **fill** sequence in `email-copy/fill-capture.md` (showed jobs, not catch-and-book)
