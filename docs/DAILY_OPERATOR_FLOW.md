...
Compare this to a full ads-plus-booking service (broader responsibility and delivery cost immediately) and standalone bespoke AI builds (useful opportunities, but a less repeatable first retainer). The narrower managed-booking test best serves the immediate learning and repeatability objective, conditional on enough existing demand and recoverable commercial value. Do not infer those conditions from an installation page alone. The active installation contract supersedes the archived breakdown targeting and Search-linked guarantee. The runner now rejects historical campaign fallback. Do not reuse retired repair tokens or promises.
...
```mermaid
flowchart TD
    G[Business goals, commitments and constraints] --> D
    S[Live systems and changes since last review] --> V[Check freshness, coverage and meaning]
    V --> D[Diagnose the current bottleneck]
    K[Relevant research and previous experiments] --> D
    D --> P[Choose work; define success, owner and review time]
    P --> A[Agent executes authorised work]
    P --> J[Jules handles decisions and human work]
    A --> C[Verify what actually changed]
    J --> C
    C --> O[Observe results when enough time has passed]
    O --> L[Update evidence and decide what to keep or change]
    L --> D
    L --> K
    P --> B[Compass brief and work records]
    C --> B
    L --> B
```
