...
This foundation is not the complete planned release. Required follow-up: conservative legacy identity/membership bridge; full CRM social network and phone classification schema; canonical international writer preservation; adapter/tool availability checks and executable provider integration; automatic all-suitable candidate selection with selected role/attribution rules; general reusable frozen selections; chunked multi-list template application with impact preview; current draft/verification/suppression approval binding; exports; paused upload and resumable actual-provider reconciliation; MCP tools; hosted migration/flag rollout and real-scale acceptance. Cache/missing-field policy is persisted for the executor but this service does not fetch pages or certify that an external executor used the cache.
...
`preview_apply` data is `{current_list_id,list_ids?,template_version_id}` and expected revision zero. Omitted lists default to current list. It freezes every existing current draft across those lists, its exact prior ID/revision, company input revision, and usable signal inputs. Config reports manual_count. `apply_chunk` uses empty data and the current job revision; it renders and commits at most 100 frozen items through the same draft transaction. Concurrent draft/evidence changes become conflicts; missing required signals become failures. Existing manual copy is replaced while exact history remains. AI template jobs explicitly require an AI executor instead of falling back to deterministic output.
