-- Keep Pathfinder and operating work connected in one transaction.
CREATE OR REPLACE FUNCTION public.compass_operating_save(p jsonb, p_actor text) RETURNS jsonb
...
BEGIN
 ...
 IF p->>'action' IN ('task','project') THEN
  ...
  END IF;
  IF nullif(p->'context'->>'goal_id','') IS NOT NULL THEN
   INSERT INTO compass_pathfinder_links(goal_id,work_type,work_id,relation,state,rationale,actor)
   VALUES(p->'context'->>'goal_id',p->>'action',coalesce(result->'task'->>'id',result->'project'->>'id'),'contributes',CASE WHEN p->'context'->>'state'='proposed' THEN 'proposed' ELSE 'active' END,p->'context'->>'reason',p_actor)
   ON CONFLICT(goal_id,work_type,work_id,relation) DO UPDATE SET state=excluded.state,rationale=excluded.rationale,updated_at=stamp;
  END IF;
 ELSE
