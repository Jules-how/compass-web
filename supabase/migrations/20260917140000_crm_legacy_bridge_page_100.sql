-- Raise the bounded legacy bridge from 25 to 100 rows per packet.
CREATE OR REPLACE FUNCTION public.crm_legacy_page(p_after text DEFAULT '', p_until text DEFAULT NULL, p_limit integer DEFAULT 25)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE ceiling text; rows jsonb; total bigint; remaining bigint;
BEGIN
 IF p_limit NOT BETWEEN 1 AND 100 THEN RAISE EXCEPTION 'crm_invalid_limit'; END IF;
...
END $$;
CREATE OR REPLACE FUNCTION public.crm_legacy_apply(p_request_id text, p_hash text, p_actor text, p_packets jsonb, p_rows jsonb)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE prior crm_research_receipts; r jsonb; p jsonb; result jsonb; results jsonb:='[]'; member_count integer; row_count integer:=0;
BEGIN
 IF jsonb_array_length(p_rows) NOT BETWEEN 1 AND 100 OR jsonb_array_length(p_packets)>100 THEN RAISE EXCEPTION 'crm_invalid_import_size'; END IF;
