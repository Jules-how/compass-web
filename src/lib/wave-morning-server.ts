...
export async function loadMorningWavePayload(
  ...
  return {
    ...
    recommendation: reviewState === 'current' ? (todayRes.data?.recommendation ?? acceptedRes.data?.recommendation ?? null) : null,
    homeBlurb: reviewState === 'current' ? setup.homeBlurb : 'Previous guidance needs a new review. Use current outreach actions and provider state.',
    writeup: reviewState === 'current' ? setup.writeup : null,
