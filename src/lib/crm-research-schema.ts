...
export const CRM_METHOD_TYPES = ['email', 'phone', 'linkedin', 'contact_form', 'facebook', 'instagram', 'x', 'youtube', 'tiktok', 'whatsapp', 'other_social'] as const
const timezone = z.string().max(100).refine(value => {
  try { new Intl.DateTimeFormat('en', { timeZone: value }); return true } catch { return false }
}, 'Use an IANA timezone').nullable().optional()
const id = z.string().min(1).max(160).regex(/^[a-zA-Z0-9_:.-]+$/)
/** lead_contacts.id values, including Instantly mailbox ids such as inst-mail-accounts@example.test */
export const crmLeadId = z.string().min(1).max(160).regex(/^[a-zA-Z0-9_@:.-]+$/)
...
export const crmRecordSchemas = {
  ...
  location: z.strictObject({ id, company_id: id, label: text, kind: z.enum(['premises', 'service_area']), address: nullableText, city: nullableText, state: z.enum(['NSW', 'VIC', 'QLD', 'WA', 'SA', 'TAS', 'ACT', 'NT']).nullable().default(null), regions: z.array(z.enum(CRM_REGIONS)).max(5).default([]), country_code: z.string().regex(/^[A-Z]{2}$/).nullable().optional(), administrative_region: z.string().max(200).nullable().optional(), suburb: z.string().max(200).nullable().optional(), postcode: z.string().max(40).nullable().optional(), timezone, source_id: id, observed_at: optionalTime, status: z.enum(['active', 'closed', 'unknown']).default('unknown') }),
  ...
  method: z.strictObject({ id, method_type: z.enum(CRM_METHOD_TYPES), value: text, normalized_value: text, normalization_version: z.literal(1).default(1) }),
  candidate: z.strictObject({ id, company_id: id, method_id: id, affiliation_id: optionalId, location_id: optionalId, purpose: z.enum(['general', 'personal_work', 'department', 'unknown']).default('unknown'), phone_kind: z.enum(['mobile', 'work', 'business', 'unknown']).nullable().optional(), first_origin: z.enum(['published_general', 'published_personal_work', 'provider_enriched', 'generated_hypothesis', 'legacy_unknown']), state: z.enum(['retained', 'unresolved', 'rejected']).default('unresolved'), reason: z.string().max(2000).default(''), generation: z.strictObject({ generator_version: text, format_ids: z.array(text).min(1).max(20), name_tokens: z.array(text).min(1).max(10), domain: text }).nullable().default(null) }),
  ...
  lead_link: z.strictObject({ id, lead_id: crmLeadId, company_id: id, affiliation_id: optionalId, location_id: optionalId, primary_email_candidate_id: optionalId, primary_phone_candidate_id: optionalId, match_state: z.enum(['proposed', 'confirmed', 'rejected']), reason: text, source_id: id, expected_lead_updated_at: optionalTime }),
...
export function parseCrmCommand(input: unknown): CrmCommand {
  ...
  for (const [i, op] of envelope.data.operations.entries()) {
    ...
    if (op.kind === 'method') {
      ...
      if (!['email', 'phone'].includes(String(r.method_type)) && !url.safeParse(v).success) fail('value', 'HTTP URL required')
    }
