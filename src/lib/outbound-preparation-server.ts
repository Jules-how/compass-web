...
export async function createPreparationRun(
  ...
  for (const [index, row] of sourceRows.entries()) {
    ...
    const saved = await db
      .from('compass_outbound_companies')
      .upsert(
        {
          id: companyId,
          identity_key: key,
...
export async function getPreparationState(
  ...
  const [approvals, loads] = pids.length
    ? await Promise.all([
        ...
        db.from('compass_outbound_loads').select('*').in('preparation_id', pids)
      ])
    : [
        { data: [], error: null },
        { data: [], error: null }
...
export async function reconcileBrowserLoad(db: SupabaseClient, id: string) {
  ...
  const after = await instantlyGetCampaign(key, load.data.instantly_campaign_id)
