...
export function validatePlanning(kind, input) {
  ...
  if (kind === "goal") {
    ...
    const regular = number(input.regular, true),
      stretch = number(input.stretch);
    if (stretch != null && stretch < regular)
      throw new Error("Stretch must be at least the regular target.");
...
export function goalProgress(goal) {
  if (goal.actual == null || !goal.regular) return null;
  return Math.max(
    0,
    Math.min(100, Math.round((goal.actual / goal.regular) * 100)),
  );
}
