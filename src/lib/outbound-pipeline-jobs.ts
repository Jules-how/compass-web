...
export type PipelineJobItem={id:string;job_id:string;status:'queued'|'applied'|'conflicted'|'failed';payload:{company_id?:string;recipient_id?:string;list_id?:string;previous_id?:string|null;previous_revision?:number;manual?:boolean;signals?:Record<string,string>;input_refs?:string[];row?:Record<string,unknown>};result:Record<string,unknown>}
...
export type ExportData={list_id:string;grain:'companies'|'recipients';columns:string[];filters?:Record<string,string>;company_ids?:string[];recipient_ids?:string[]}
...
export const RECIPIENT_EXPORT_COLUMNS=['id','list_id','company_id','company_name','name','role','mailbox','mailbox_result','checked_at','suppressed','eligibility','current_draft_id','subject','opener','body','cta','unsubscribe','copy_control'] as const
...
export type MembershipPreviewData={list_id:string;operation:'add'|'remove';filters?:Record<string,string>;company_ids?:string[];recipient_ids?:string[]}
