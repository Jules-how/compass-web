        ...
        <section className={styles.section}><h3>Campaigns</h3><p className={styles.meta}>Provider activity and preparation receipts are separate. Loaded receipts are not added to the provider’s recipient total.</p>
          ...
          {data.campaigns.map(c => <article className={styles.campaign} id={`outbound-campaign-${c.id}`} key={c.id}>
            ...
            <dl className={styles.metrics}>
              {[["First contacts prepared", c.first_contact_count], ["Follow-up drafts", c.followup_count], ["Loaded (receipts)", c.loaded_receipt_count], ["Loaded (provider)", c.provider?.loaded], ["People contacted", c.provider?.contacted], ["Emails sent", c.provider?.sent], ["Replies", c.provider?.replies]].map(([label, value]) => <div key={String(label)}><dt>{label}</dt><dd>{number(value as number | null | undefined)}</dd></div>)}
            </dl>
            {c.unclassified_prepared_count ? <p className={styles.meta}>{c.unclassified_prepared_count} other prepared recipients have no recorded purpose; they are not counted as new first contacts.</p> : null}
