...
export const PIPELINE_STAGE_INFO: Record<PipelineStage, { label: string; title: string; description: string; action: string }> = {
  list: { label: 'List', title: 'Build your working list', description: 'Filter existing companies, then add the ones you need to a list.', action: 'Select for research' },
  ...
  contacts: { label: 'Contacts', title: 'Find the right people', description: 'Open a company to inspect names, work emails, phone numbers and social profiles.', action: 'Queue contact sourcing' },
  verify: { label: 'Verify', title: 'Check email addresses', description: 'Open a company to review each email’s verification result. Phone-only leads stay available.', action: 'Queue verification' },
...
export function companyStagePresentation(company: Pick<PipelineCompany, 'stage_status' | 'reason'>, stage: PipelineStage) {
  ...
  const hasResult = Boolean(company.stage_status) && !(company.stage_status === 'held' && reason.toLowerCase() === 'not started');
  if (!hasResult) {
...
export function pipelineRunBlocker(input: {
  ...
  if (!input.workflowId) return 'Choose a saved workflow in List setup.';
  ...
  if (input.stage === 'write' && !input.templateId) return 'Choose a writing template in List setup.';
