  ...
  return <div className="op-filter-area">
    <form className="op-filter-form" onSubmit={event => { event.preventDefault(); apply(); event.currentTarget.querySelector("details")?.removeAttribute("open"); }}>
      <div className="op-filter-main">
        <Field label="Search companies"><input value={draft.q} onChange={event => setDraft({ ...draft, q: event.target.value })} placeholder="Company name" /></Field>
      ...
      <details className="op-extra-filters"><summary>More filters{extras > 0 ? ` (${extras})` : ''}</summary>
        <div className="op-extra-filter-fields">
          ...
          {stage !== 'list' && <Field label="Stage outcome"><select value={draft.status} onChange={event => setDraft({ ...draft, status: event.target.value })}><option value="">All outcomes</option>{['ready', 'held', 'completed', 'failed', 'stale'].map(item => <option key={item} value={item}>{label(item)}</option>)}</select></Field>}
          {stage === 'write' && <>
