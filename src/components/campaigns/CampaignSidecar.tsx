  ...
  function saveCampaign(patch: CampaignPatch) {
    void updateCampaign(campaignId, patch)
      .then((updated: CompassCampaign) => {
        ...
        if (patch.priority !== undefined) setPriority(updated.priority)
        if (patch.health !== undefined) setHealth(updated.health)
        if (patch.start_date !== undefined) setStartDate(updated.start_date ?? '')
        if (patch.end_date !== undefined) setEndDate(updated.end_date ?? '')
        if (patch.go_live_at !== undefined) setGoLiveLocal(goLiveToDatetimeLocal(updated.go_live_at))
        if (patch.summary !== undefined) setSummary(updated.summary ?? '')
        if (patch.owner_label !== undefined) setOwnerLabel(updated.owner_label ?? '')
