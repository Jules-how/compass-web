...
export function PlanningRecords() {
  const [kind, setKind] = useState("time"),
    ...
    [form, setForm] = useState(empty("time")),
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="compass-page-title">Time & activity</h1>
              ...
              {kind === "goal" ? (
                <>
                  <p className="mt-2 text-sm text-neutral-600">
                    {r.data.period} ·{" "}
                    {r.data.due
                      ? `due ${displayDate(r.data.due)}`
                      : "No due date"}{" "}
                    · {displayNumber(r.data.actual)} /{" "}
                    {displayNumber(r.data.regular)} {r.data.unit}
                    {r.data.stretch != null
