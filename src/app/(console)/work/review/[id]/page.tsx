...
export default function Review({params}:{params:Promise<{id:string}>}){
 ...
 useEffect(()=>{void fetch(`/api/operating/preparation/${encodeURIComponent(id)}`).then(async r=>{const b=await r.json();if(!r.ok)throw new Error(b.error);setRecord(b)}).catch(e=>setError(e.message))},[id])
