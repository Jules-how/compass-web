...
export async function POST(request: Request) {
  ...
  let body: { campaignId?: string; pushSequence?: boolean } = {}
  try {
  ...
  try {
    ...
    console.error('[agent/instantly/ensure]', err instanceof Error ? err.message : err)
    return portalJson({ error: 'ensure_failed' }, { status: 500 })
  }
}
