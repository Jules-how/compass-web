...
export async function POST(request:Request){const auth=requireAgentAuth(request);if(auth)return auth;try{const actor='agent';return portalJson(await applyPipelineJob(getPortalAdminClient(),await readBoundedJson(request,256*1024),actor));}catch(error){return pipelineErrorResponse(error)}}
