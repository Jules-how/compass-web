...
test('leads list API supports search, sync, and completeness filters', () => {
  ...
  assert.match(query, /leadFiltersNeedExactCount/)
...
test('LeadTable surfaces search, segments, bulk actions, and hides UUID by default', () => {
  ...
  assert.match(table, /useLeadGridColumns/)
  assert.match(table, /Filters/)
  assert.match(table, /columnPreset/)
...
test('lead bulk and archive use Supabase is_archived', () => {
  const bulk = read('src/app/api/leads/bulk/route.ts')
  const query = read('src/lib/leads-query.ts')
  const migration = read('supabase/migrations/0057_lead_archived.sql')
...
test('lead column registry includes Instantly-aligned fields', () => {
  ...
  assert.match(cols, /first_name/)
  assert.match(cols, /last_name/)
  assert.match(cols, /opener/)
  assert.match(cols, /lead_facts/)
  assert.match(cols, /job_title/)
  assert.match(cols, /resolveVisibleLeadColumns/)
