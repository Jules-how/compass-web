...
test('lead facts parser and per-row mark path are wired', () => {
  ...
  assert.match(mark, /emails/)
  assert.match(mark, /outbound_status/)
  assert.match(mark, /instantly_lead_id/)
  assert.match(mark, /email_verify_status/)
  assert.match(mark, /email_verified_at/)
  ...
  assert.match(cohort, /unverified_only/)
