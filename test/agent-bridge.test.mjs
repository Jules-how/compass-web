...
test('agent bridge routes and cron are wired', () => {
  ...
  assert.match(leads, /export async function POST/)
  assert.match(leads, /view === 'counts'/)
  assert.match(leads, /application\/x-ndjson/)
  assert.match(campaigns, /compactWaveForAgent/)
...
test('lead facts parser and per-row mark path are wired', () => {
  ...
  assert.match(markLib, /use_rows_for_facts/)
  assert.match(markLib, /rows/)
  assert.match(markLib, /emails/)
  assert.match(markLib, /outbound_status/)
  assert.match(markLib, /instantly_lead_id/)
  assert.match(markLib, /email_verify_status/)
  assert.match(markLib, /email_verified_at/)
  ...
  assert.match(cohort, /columns: 'cohort'/)
  assert.match(query, /lead_facts/)
  assert.match(query, /unverified_only/)
  const skill = read('.cursor/skills/compass-agent/SKILL.md')
  assert.match(skill, /leads\/cohort/)
  assert.match(skill, /lead_facts/)
  assert.match(skill, /leads\.search|\/api\/agent\/leads\?view=rows/)
})
...
test('compass-agent skill documents lean brief-first workflow', () => {
  ...
  assert.match(skill, /instantly\/push-leads/)
})
