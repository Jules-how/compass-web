...
test('wave files and sidecar Wave section are wired', () => {
  ...
  assert.match(read('src/lib/lead-mark.ts'), /opener_track/)
  ...
  assert.doesNotMatch(brief, /wave_cap/)
  assert.match(brief, /currentWave/)
  assert.match(brief, /morningWave/)
  assert.match(brief, /uncontactedRemaining/)
  assert.match(brief, /lastImportAt/)
  assert.match(brief, /deriveCurrentWave/)
