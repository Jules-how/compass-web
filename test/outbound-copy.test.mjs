...
test('copy archive types, editor tab, APIs, and migration are wired', () => {
  ...
  assert.match(store, /overlayArchiveInstantlyPerformance/)
  assert.match(store, /sortCopyArchiveEntries/)
  ...
  assert.match(panel, /Named pieces/)
  assert.match(panel, /Positive \/ sent/)
  assert.match(panel, /overlayArchiveInstantlyPerformance/)
...
test('compose desk has mixer, levers, risk and ps slots, Instantly overlay', () => {
  const copy = read('src/lib/outbound-copy.ts')
  assert.match(copy, /risk_reversal/)
  assert.match(copy, /'ps'/)
  assert.match(copy, /export function ensureSequenceSlots/)
  assert.match(copy, /risk_preview/)
  assert.match(copy, /ps_preview/)
  assert.match(copy, /positive/)

  const levers = read('src/lib/outbound-levers.ts')
  assert.match(levers, /COLD_EMAIL_QUESTIONS/)
  assert.match(levers, /COLD_EMAIL_LEVERS/)
  assert.match(levers, /Why them, why now/)
  assert.match(levers, /Segmentation/)
  assert.match(levers, /Risk reversal/)

  const craft = read('src/components/outbound/ComposeCraftPanel.tsx')
  assert.match(craft, /ComposeRecipeStrip/)
  assert.match(craft, /ComposeLeversPanel/)
  assert.match(craft, /ComposeRatesStrip/)
  assert.match(craft, /Meetings \/ 100/)

  const editor = read('src/components/outbound/SequenceEditor.tsx')
  assert.match(editor, /ComposeLeversPanel/)
  assert.match(editor, /ComposeRecipeStrip/)
  assert.match(editor, /ensureSequenceSlots/)
  assert.match(editor, /railTab === 'levers'/)
  assert.match(editor, /risk_reversal/)
})
